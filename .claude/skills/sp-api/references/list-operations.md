...
The current `lists.items` command supports `--select` and `--top`. More complex OData filtering, CAML, fields, and views should become semantic verbs before agents rely on them. Do not fall back to raw HTTP.
